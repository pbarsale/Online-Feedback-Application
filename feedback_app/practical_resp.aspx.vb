...
Partial Class practical_resp
    Inherits System.Web.UI.Page

    Dim conn1 As SqlConnection
    Dim constr1 As String
    Dim cmd, cmd1, cmd2 As SqlCommand
    Dim reader, reader2, reader1 As SqlDataReader
    ...
    Public sem1 As String
    Dim row_id As Integer
    Dim i, j As Integer
    Dim dept As String
    Dim grade, e As String
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        constr1 = ConfigurationManager.ConnectionStrings("feedbackConnectionString").ConnectionString.ToString()
        conn1 = New SqlConnection(constr1)
        conn1.Open()

        cmdstr1 = "select s.course_code,s.staff_name,c.degree from staff_practical s,course_info c where s.dept_name='" & dept & "' and s.sem=" & semester & " and c.course_code=s.course_code and c.degree='" & Session("degree") & "' and c.iselective=0 and c.annual_year='" & Session("annual_year") & "'"
        cmd1 = New SqlCommand(cmdstr1, conn1)
        ...
        reader1.Close()
        cmdstr1 = "select staff_name from staff_practical  where dept_name='" & dept & "' and sem=" & semester & " and course_code='" & Session("prac_course_code") & "' and annual_year='" & Session("annual_year") & "'"
        cmd1 = New SqlCommand(cmdstr1, conn1)
        reader1 = cmd1.ExecuteReader

        If reader1.Read() Then
            subject(count + 1) = Session("prac_course_code")
            staff(count + 1) = reader1("staff_name")
            count = count + 1
        End If
        reader1.Close()
        ...
        For Me.i = 1 To count
            ...
            cmd2 = New SqlCommand(cmdstr2, conn1)
        ...
        cmdstr = "select question_name from question_info where question_type='practical' order by question_id"
        cmd = New SqlCommand(cmdstr, conn1)
        ...
        cmdstr = "select question_name from question_info where question_type='PR_impression'"
        cmd = New SqlCommand(cmdstr, conn1)
        reader = cmd.ExecuteReader
    ...
    Protected Sub btnsubmit_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnsubmit.Click
        ...
        If flag = 0 Then
            ...
            For Me.i = 1 To count
                ...
                cmdstr = "insert into practical(course_code,stud_response,PR_impression,total,percentage,sys_date,dept_name,sem,grade,annual_year) values('" & subject(i) & "','" & stud_resp & "','" & combo(i, j).SelectedValue & "'," & total & "," & percent & ",'" & DateAndTime.Now.ToString("MM/dd/yyyy") & "','" & dept & "'," & semester & ",'" & grade & "','" & Session("annual_year") & "')"
                cmd = New SqlCommand(cmdstr, conn1)
                cmd.ExecuteNonQuery()
            ...
            If txt.Text <> "" Then
                ...
                str = String.Concat("", txt.Text)
                str = str.Replace("'", " ")
                cmdstr = "insert into comments(dept_name,comment,annual_year,annual_sem)values('" & dept & "','" & str & "','" & Session("annual_year") & "'," & Session("annual_sem") & ")"
                cmd = New SqlCommand(cmdstr, conn1)
                cmd.ExecuteNonQuery()
            ...
            conn1.Close()
            ' MsgBox("Information Saved Successfully..Thank You for your Feedback !!!", MsgBoxStyle.MsgBoxSetForeground + MsgBoxStyle.Information, "Feedback Form")
            Response.Redirect("submit.aspx")

        Else
            ' MsgBox("Please select all the fields", MsgBoxStyle.MsgBoxSetForeground + MsgBoxStyle.Exclamation, "Feedback Form")
